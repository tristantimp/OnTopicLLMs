...
for entry_idx, row in filtered_data.iterrows():
    # Print system instructions before distractor pairs for context
    ...
    for pair_idx, d in enumerate(distractors):
        ...
        # Look through all conversation blocks and turns to find user turn matching distractor question
        found_response = None
        for conv_block in conversation_blocks:
            for i in range(len(conv_block) - 1):
                if conv_block[i]['role'] == 'user' and conv_block[i]['content'].strip() == distractor_question:
                    # Next turn should be assistant response to distractor user turn
...
with open(f"annotations_{selected_domain}_distractor_pairs.json", "w", encoding="utf-8") as f:
    json.dump(annotations, f, indent=2, ensure_ascii=False)


print(f"\nSaved annotations to annotations_{selected_domain}_distractor_pairs.json")